...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (registerForm) {
        ...
        registerForm.addEventListener('submit', function(e) {
            ...
            // Validaciones
            if (!fullName || !username || !email || !password || !petType) {
                showNotification('Por favor completa todos los campos', 'error');
                return;
            }
            
            if (!validateUsername(username)) {
                showNotification('El nombre de usuario solo puede contener letras, números y guiones bajos', 'error');
                return;
            }
            ...
            // Guardar datos temporalmente y continuar
            const userData = {
                fullName: fullName,
                username: username,
                email: email,
            ...
            // Simular guardado temporal (en localStorage)
            localStorage.setItem('tempUserData', JSON.stringify(userData));
            
            // Mostrar carga y redireccionar
            const registerBtn = registerForm.querySelector('button[type="submit"]');
            const originalText = registerBtn.innerHTML;
            
            registerBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Continuando...';
